...
public class ArgbBar extends View {
	...
	private static final boolean ORIENTATION_DEFAULT = ArgbBar.ORIENTATION_HORIZONTAL;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int measureSpec;
		if (mOrientation == ArgbBar.ORIENTATION_HORIZONTAL) {
		...
		if (mOrientation == ArgbBar.ORIENTATION_VERTICAL) {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		switch (mColorComponent) {
		case ALPHA:
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
					Color.TRANSPARENT, Color.WHITE}, null, Shader.TileMode.CLAMP);
		case RED:
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
					Color.BLACK, Color.RED}, null, Shader.TileMode.CLAMP);
		case GREEN:
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
					Color.BLACK, Color.GREEN}, null, Shader.TileMode.CLAMP);
		case BLUE:
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
					Color.BLACK, Color.BLUE}, null, Shader.TileMode.CLAMP);
		}
		mBarPaint.setShader(shader);
		mPositionToValueFactor = 0xFF / (float) mBarLength;
		mValueToPositionFactor = (float) mBarLength / 0xFF;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mPicker != null) {
				...
				case RED:
					mPicker.setNewRed(mCurrentValue);
				case GREEN:
					mPicker.setNewGreen(mCurrentValue);
				case BLUE:
					mPicker.setNewBlue(mCurrentValue);
				}
			}
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsMovingPointer) {
				if (mTouchPositionInBarAndBarPointerPositionDelta <= -mBarPointerHaloRadius
					...
					switch (mColorComponent) {
					case ALPHA:
						mBarPointerPaint.setColor(Color.argb(mCurrentValue, 0xFF, 0xFF,
								0xFF));
						if (mPicker != null) {
							mPicker.setNewAlpha(mCurrentValue);
						}
					case RED:
						mBarPointerPaint.setColor(Color.argb(0xFF, mCurrentValue, 0, 0));
						if (mPicker != null) {
							mPicker.setNewRed(mCurrentValue);
						}
						invalidate();
					case GREEN:
						mBarPointerPaint.setColor(Color.argb(0xFF, 0, mCurrentValue, 0));
						if (mPicker != null) {
							mPicker.setNewGreen(mCurrentValue);
						}
					case BLUE:
						mBarPointerPaint.setColor(Color.argb(0xFF, 0, 0, mCurrentValue));
						if (mPicker != null) {
							mPicker.setNewBlue(mCurrentValue);
						}
					}
	...
	public void setValue(int value) {
		...
		case RED:
			mBarPointerPaint.setColor(Color.argb(0xFF, value, 0, 0));
		case GREEN:
			mBarPointerPaint.setColor(Color.argb(0xFF, 0, value, 0));
		case BLUE:
			mBarPointerPaint.setColor(Color.argb(0xFF, 0, 0, value));
		}
		invalidate();
	}
	...
	protected Parcelable onSaveInstanceState() {
		...
		state.putParcelable(ArgbBar.STATE_PARENT, superState);
		state.putInt(ArgbBar.STATE_VALUE, getValue());
		return state;
	}
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		Parcelable superState = savedState.getParcelable(ArgbBar.STATE_PARENT);
		super.onRestoreInstanceState(superState);
		setValue(savedState.getInt(ArgbBar.STATE_VALUE));
	}
}