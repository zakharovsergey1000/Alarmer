...
public class SVBar extends View {
	...
	private static final boolean ORIENTATION_DEFAULT = SVBar.ORIENTATION_HORIZONTAL;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int measureSpec;
		if (mOrientation == SVBar.ORIENTATION_HORIZONTAL) {
		...
		if (mOrientation == SVBar.ORIENTATION_VERTICAL) {
			setMeasuredDimension(barPointerHaloRadiusx2, mBarLength
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (!isInEditMode()) {
			shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
					0xffffffff, Color.HSVToColor(mHSVColor), 0xff000000}, null,
		...
		if (hsvColor[1] < hsvColor[2]) {
			...
			mBarPointerPosition = Math.round(mSVToPosFactor * (1 - hsvColor[2])
	...
	public void setColor(int color) {
		...
		shader = new LinearGradient(mBarPointerHaloRadius, 0, x1, y1, new int[] {
				Color.WHITE, color, Color.BLACK}, null, Shader.TileMode.CLAMP);
		mBarPaint.setShader(shader);
		calculateColor(mBarPointerPosition);
		mBarPointerPaint.setColor(mColor);
		if (mPicker != null) {
	...
	private void calculateColor(int coord) {
		...
		if (coord > mBarLength / 2 && coord < mBarLength) {
			...
			mColor = Color.HSVToColor(new float[] {mHSVColor[0], mPosToSVFactor * coord,
					1f});
	 ...
	 * <br>
	 * WARNING: Don't change the color picker. it is done already when the bar is added to
	 * the ColorPicker
	 *
	...
	protected Parcelable onSaveInstanceState() {
		...
		state.putParcelable(SVBar.STATE_PARENT, superState);
		state.putFloatArray(SVBar.STATE_COLOR, mHSVColor);
		float[] hsvColor = new float[3];
		Color.colorToHSV(mColor, hsvColor);
		if (hsvColor[1] < hsvColor[2]) {
			state.putFloat(SVBar.STATE_SATURATION, hsvColor[1]);
		} else {
			state.putFloat(SVBar.STATE_VALUE, hsvColor[2]);
		}
		return state;
	}
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		Parcelable superState = savedState.getParcelable(SVBar.STATE_PARENT);
		super.onRestoreInstanceState(superState);
		setColor(Color.HSVToColor(savedState.getFloatArray(SVBar.STATE_COLOR)));
		if (savedState.containsKey(SVBar.STATE_SATURATION)) {
			setSaturation(savedState.getFloat(SVBar.STATE_SATURATION));
		} else {
			setValue(savedState.getFloat(SVBar.STATE_VALUE));
		}
	}
}