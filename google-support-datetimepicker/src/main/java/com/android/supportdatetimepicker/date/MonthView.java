...
/** A calendar-like view displaying a specified month and the appropriate selectable day
 * numbers within the specified month. */
public abstract class MonthView extends View {
	private static final String TAG = "MonthView";
	/** These params can be passed into the view to control how it appears.
	 * {@link #VIEW_PARAMS_WEEK} is the only required field, though the default values are
	 * unlikely to fit most layouts correctly. */
	/** This sets the height of this week in pixels */
	public static final String VIEW_PARAMS_HEIGHT = "height";
	/** This specifies the position (or weeks since the epoch) of this week, calculated
	 * using {@link Utils#getWeeksSinceEpochFromJulianDay} */
	...
	protected int mNumRows = MonthView.DEFAULT_NUM_ROWS;
	...
	public MonthView(Context context, AttributeSet attr) {
		...
		mRowHeight = (res
				.getDimensionPixelOffset(R.dimen.date_picker_view_animator_height) - getMonthHeaderSize())
	...
	/** Sets up the text and style properties for painting. Override this if you want to
	 * use a different paint. */
	protected void initView() {
		mMonthTitlePaint = new Paint();
		mMonthTitlePaint.setFakeBoldText(true);
		mMonthTitlePaint.setAntiAlias(true);
		mMonthTitlePaint.setTextSize(MonthView.MONTH_LABEL_TEXT_SIZE);
		...
		mSelectedCirclePaint.setAlpha(MonthView.SELECTED_CIRCLE_ALPHA);
		mMonthDayLabelPaint = new Paint();
		mMonthDayLabelPaint.setAntiAlias(true);
		mMonthDayLabelPaint.setTextSize(MonthView.MONTH_DAY_LABEL_TEXT_SIZE);
		...
		mMonthNumPaint.setTextSize(MonthView.MINI_DAY_NUMBER_TEXT_SIZE);
	...
	/** Sets all the parameters for displaying this week. The only required parameter is
	 * the week number. Other parameters have a default value and will only update if a
	 * new value is included, except for focus month, which will always default to no
	 * focus month if no value is passed in. See {@link #VIEW_PARAMS_HEIGHT} for more info
	 * on parameters.
	 *
	 * @param params
	...
	public void setMonthParams(HashMap<String, Integer> params) {
		...
		if (params.containsKey(MonthView.VIEW_PARAMS_HEIGHT)) {
			mRowHeight = params.get(MonthView.VIEW_PARAMS_HEIGHT);
			if (mRowHeight < MonthView.MIN_HEIGHT) {
				mRowHeight = MonthView.MIN_HEIGHT;
			}
		}
		if (params.containsKey(MonthView.VIEW_PARAMS_SELECTED_DAY)) {
			mSelectedDay = params.get(MonthView.VIEW_PARAMS_SELECTED_DAY);
		}
		// Allocate space for caching the day numbers and focus values
		mMonth = params.get(MonthView.VIEW_PARAMS_MONTH);
		mYear = params.get(MonthView.VIEW_PARAMS_YEAR);
		...
		if (params.containsKey(MonthView.VIEW_PARAMS_WEEK_START)) {
			mWeekStart = params.get(MonthView.VIEW_PARAMS_WEEK_START);
		} else {
	...
	public void reuse() {
		mNumRows = MonthView.DEFAULT_NUM_ROWS;
		requestLayout();
	}
	...
	private String getMonthAndYearString() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			return DateUtils.formatDateRange(getContext(), mFormatter, millis, millis,
					flags, Time.getCurrentTimezone()).toString();
		} else {
			return new SimpleDateFormat("MMMM").format(mCalendar.getTime()) + " "
					+ new SimpleDateFormat("yyyy").format(mCalendar.getTime());
		}
	}

	protected void drawMonthTitle(Canvas canvas) {
		int x = (mWidth + 2 * mEdgePadding) / 2;
		int y = (getMonthHeaderSize() - MonthView.MONTH_DAY_LABEL_TEXT_SIZE) / 2
	...
	protected void drawMonthDayLabels(Canvas canvas) {
		...
		for (int i = 0; i < mNumDays; i++) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				...
				canvas.drawText(
						new SimpleDateFormat("EEEE").format(mDayLabelCalendar.getTime())
	...
	/** Draws the week and month day numbers for this week. Override this method if you
	 * need different placement.
	 *
	 * @param canvas
	 *            The canvas to draw on */
	protected void drawMonthNums(Canvas canvas) {
		int y = (mRowHeight + MonthView.MINI_DAY_NUMBER_TEXT_SIZE) / 2
				- MonthView.DAY_SEPARATOR_WIDTH + getMonthHeaderSize();
		final float dayWidthHalf = (mWidth - mEdgePadding * 2) / (mNumDays * 2.0f);
		int j = findDayOffset();
		for (int dayNumber = 1; dayNumber <= mNumCells; dayNumber++) {
			final int x = (int) ((2 * j + 1) * dayWidthHalf + mEdgePadding);
			int yRelativeToDay = (mRowHeight + MonthView.MINI_DAY_NUMBER_TEXT_SIZE) / 2
			...
			drawMonthDay(canvas, mYear, mMonth, dayNumber, x, y, startX, stopX, startY,
					stopY);
			j++;
			if (j == mNumDays) {
				j = 0;
	...
	public abstract void drawMonthDay(Canvas canvas, int year, int month, int day, int x,
			int y, int startX, int stopX, int startY, int stopY);
	 ...
	 * Returns the day or -1 if the position wasn't in a day.
	 *
	 * @param x
	...
	/** @return true if the specified year/month/day are within the range set by minDate and
	 *         maxDate. If one or either have not been set, they are considered as
	...
	/** Clears accessibility focus within the view. No-op if the view does not contain
	 * accessibility focus. */
	 ...
	 * @return {@code false} if the date is not valid for this month view, or {@code true}
	...
	protected class MonthViewTouchHelper extends ExploreByTouchHelper {
		...
		/** Generates a description for a given time object. Since this description will be
		 * spoken, the components are ordered by descending specificity as DAY MONTH YEAR.
		 *
		 * @param day
		...
		protected CharSequence getItemDescription(int day) {
			...
			final CharSequence date = DateFormat.format(MonthViewTouchHelper.DATE_FORMAT,
