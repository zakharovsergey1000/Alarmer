...
public abstract class DayPickerView extends ListView implements OnScrollListener,
    ...
    /**
     * Creates a new adapter if necessary and sets up its parameters. Override this method
    ...
    /* Sets all the required fields for the list view. Override this method to set a
     * different list view behavior. */
    ...
    /**
     * This moves to the specified time in the view. If the time is not already in range
     * it will move the list so that the first of the month containing the time is at the
     * top of the view. If the new time is already in view the list will not be scrolled
     * unless forceScroll is true. This time may optionally be highlighted as selected as
     * well.
     *
    ...
    public boolean goTo(CalendarDay day, boolean animate, boolean setSelected,
        ...
        do {
            ...
            if (Log.isLoggable(DayPickerView.TAG, Log.DEBUG)) {
                Log.d(DayPickerView.TAG, "child at " + (i - 1) + " has top " + top);
            }
        } while (top < 0);
        ...
        if (position != selectedPosition || forceScroll) {
            ...
            if (animate) {
                smoothScrollToPositionFromTop(position, DayPickerView.LIST_TOP_OFFSET,
    ...
    /**
     * Sets the month displayed at the top of this view based on time. Override to add
     * custom events when the title is changed.
     */
    ...
    protected class ScrollStateRunnable implements Runnable {
        ...
        /**
         * Sets up the runnable with a short delay in case the scroll state immediately
         * changes again.
         *
        ...
        public void doScrollStateChange(AbsListView view, int scrollState) {
            ...
            mHandler.postDelayed(this, DayPickerView.SCROLL_CHANGE_DELAY);
        }
        ...
        public void run() {
            ...
            if (Log.isLoggable(DayPickerView.TAG, Log.DEBUG)) {
                Log.d(DayPickerView.TAG, "new scroll state: " + mNewState
                    + " old state: " + mPreviousScrollState);
            }
            ...
            if (mNewState == OnScrollListener.SCROLL_STATE_IDLE
                ...
                if (scroll && top < DayPickerView.LIST_TOP_OFFSET) {
                    if (bottom > midpoint) {
                        ...
                        smoothScrollBy(bottom, DayPickerView.GOTO_SCROLL_DURATION);
                    }
                }
            } else {
                mPreviousScrollState = mNewState;
     ...
     *
     * @return The date that has accessibility focus, or {@code null} if no date has
     * focus.
     */
    ...
    /**
     * Attempts to restore accessibility focus to a given date. No-op if {@code day} is
     * {@code null}.
     *
    ...
    private static String getMonthAndYearString(CalendarDay day) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            sbuf.append(new SimpleDateFormat("MMMM").format(cal.getTime()).toUpperCase(
                Locale.getDefault()));
        }
        sbuf.append(" ");
    ...
    /**
     * When scroll forward/backward events are received, announce the newly scrolled-to
     * month.
     */
    @SuppressLint("NewApi")
    @Override
    public boolean performAccessibilityAction(int action, Bundle arguments) {
        ...
        if (action == AccessibilityNodeInfo.ACTION_SCROLL_FORWARD) {
            ...
            // If the view is fully visible, jump one month back. Otherwise, we'll just
            // jump
        ...
        // Go to that month.
        Utils.tryAccessibilityAnnounce(this, DayPickerView.getMonthAndYearString(day));
