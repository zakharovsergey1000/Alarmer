...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	public void initialize(Context context,
			HapticFeedbackController hapticFeedbackController, int initialHoursOfDay,
		...
		if (!mHideAmPm) {
			mAmPmCirclesView.initialize(context,
					initialHoursOfDay < 12 ? RadialPickerLayout.AM
							: RadialPickerLayout.PM);
		...
		for (int i = 0; i < 12; i++) {
			hoursTexts[i] = is24HourMode ? String.format("%02d", hours24[i]) : String
					.format("%d", hours12[i]);
			innerHoursTexts[i] = String.format("%d", hours12[i]);
			minutesTexts[i] = String.format("%02d", minutes[i]);
		}
		mHourRadialTextsView.initialize(res, hoursTexts, is24HourMode ? innerHoursTexts
				: null, mHideAmPm, true);
		...
		setValueForItem(RadialPickerLayout.HOUR_INDEX, initialHoursOfDay);
	...
	private void setItem(int index, int value) {
		if (index == RadialPickerLayout.HOUR_INDEX) {
	...
	private int getCurrentlyShowingValue() {
		...
		if (currentIndex == RadialPickerLayout.HOUR_INDEX) {
			return mCurrentHoursOfDay;
	...
	public int getIsCurrentlyAmOrPm() {
		if (mCurrentHoursOfDay < 12) {
			return RadialPickerLayout.AM;
		} else if (mCurrentHoursOfDay < 24) {
			return RadialPickerLayout.PM;
		}
		return -1;
	}
	...
	private void setValueForItem(int index, int value) {
		...
		} else if (index == RadialPickerLayout.AMPM_INDEX) {
			if (!mIs24HourMode) {
				if (value == RadialPickerLayout.AM) {
					mCurrentHoursOfDay = mCurrentHoursOfDay % 12;
				} else if (value == RadialPickerLayout.PM) {
	...
	public void setAmOrPm(int amOrPm) {
		...
		mAmPmCirclesView.invalidate();
		setValueForItem(RadialPickerLayout.AMPM_INDEX, amOrPm);
	}
	...
	private int reselectSelector(int degrees, boolean isInnerCircle,
		...
		if (currentShowing == RadialPickerLayout.HOUR_INDEX) {
		...
		if (currentShowing == RadialPickerLayout.HOUR_INDEX && mIs24HourMode
	...
	private int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal,
		...
		if (currentItem == RadialPickerLayout.HOUR_INDEX) {
	...
	public int getCurrentItemShowing() {
		if (mCurrentItemShowing != RadialPickerLayout.HOUR_INDEX
	...
	public void setCurrentItemShowing(int index, boolean animate) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && animate
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				...
				int hourAlpha = index == RadialPickerLayout.HOUR_INDEX ? View.VISIBLE
						: View.INVISIBLE;
				int minuteAlpha = index == RadialPickerLayout.MINUTE_INDEX ? View.VISIBLE
						: View.INVISIBLE;
				mHourRadialTextsView.setVisibility(hourAlpha);
				mHourRadialSelectorView.setVisibility(hourAlpha);
				mMinuteRadialTextsView.setVisibility(minuteAlpha);
				mMinuteRadialSelectorView.setVisibility(minuteAlpha);
			}
		}
	}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mIsTouchingAmOrPm == RadialPickerLayout.AM
					|| mIsTouchingAmOrPm == RadialPickerLayout.PM) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsTouchingAmOrPm == RadialPickerLayout.AM
				...
				int isTouchingAmOrPm = mAmPmCirclesView.getIsTouchingAmOrPm(eventX,
						eventY);
			...
			if (mIsTouchingAmOrPm == RadialPickerLayout.AM
				...
				if (isTouchingAmOrPm == mIsTouchingAmOrPm) {
					...
					if (getIsCurrentlyAmOrPm() != isTouchingAmOrPm) {
						mListener.onValueSelected(RadialPickerLayout.AMPM_INDEX,
								mIsTouchingAmOrPm, false);
						setValueForItem(RadialPickerLayout.AMPM_INDEX, isTouchingAmOrPm);
					}
				}
				mIsTouchingAmOrPm = -1;
				break;
			...
			if (mDownDegrees != -1) {
				...
				if (degrees != -1) {
					value = reselectSelector(degrees, isInnerCircle[0], !mDoingMove,
							false);
					if (getCurrentItemShowing() == RadialPickerLayout.HOUR_INDEX
							&& !mIs24HourMode) {
						int amOrPm = getIsCurrentlyAmOrPm();
						if (amOrPm == RadialPickerLayout.AM && value == 12) {
							value = 0;
						} else if (amOrPm == RadialPickerLayout.PM && value != 12) {
							value += 12;
						}
					}
	...
	public boolean performAccessibilityAction(int action, Bundle arguments) {
		...
		if (changeMultiplier != 0) {
			...
			if (currentItemShowing == RadialPickerLayout.HOUR_INDEX) {
				stepSize = RadialPickerLayout.HOUR_VALUE_TO_DEGREES_STEP_SIZE;
				value %= 12;
