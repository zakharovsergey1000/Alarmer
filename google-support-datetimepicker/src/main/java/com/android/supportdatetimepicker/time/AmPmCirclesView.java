...
public class AmPmCirclesView extends View {
    ...
    public void initialize(Context context, int amOrPm) {
        if (mIsInitialized) {
            Log.e(AmPmCirclesView.TAG, "AmPmCirclesView may only be initialized once.");
            return;
        }
        ...
        mSelectedAlpha = AmPmCirclesView.SELECTED_ALPHA;
    ...
    void setTheme(Context context, boolean themeDark) {
        ...
        if (themeDark) {
            ...
            mSelectedAlpha = AmPmCirclesView.SELECTED_ALPHA_THEME_DARK;
        } else {
            mUnselectedColor = res.getColor(R.color.white);
            mSelectedColor = res.getColor(R.color.blue);
            mAmPmTextColor = res.getColor(R.color.ampm_text_color);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (!mDrawValuesReady) {
            ...
            // Line up the vertical center of the AM/PM circles with the bottom of the
            // main circle.
        ...
        int pmAlpha = 255;
        if (mAmOrPm == AmPmCirclesView.AM) {
            amColor = mSelectedColor;
            amAlpha = mSelectedAlpha;
        } else if (mAmOrPm == AmPmCirclesView.PM) {
            pmColor = mSelectedColor;
            pmAlpha = mSelectedAlpha;
        }
        if (mAmOrPmPressed == AmPmCirclesView.AM) {
            amColor = mSelectedColor;
            amAlpha = mSelectedAlpha;
        } else if (mAmOrPmPressed == AmPmCirclesView.PM) {
            pmColor = mSelectedColor;
            pmAlpha = mSelectedAlpha;
        }
