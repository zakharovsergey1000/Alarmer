...
/** View to show what number is selected. This will draw a blue circle over the number,
 * with a blue line coming from the center of the main circle to the edge of the blue
 * selection. */
public class RadialSelectorView extends View {
	 ...
	 *            Current context.
	 * @param is24HourMode
	 *            Whether the selector is in 24-hour mode, which will tell us whether the
	 *            circle's center is moved up slightly to make room for the AM/PM circles.
	 * @param hasInnerCircle
	 *            Whether we have both an inner and an outer circle of numbers that may be
	 *            selected. Should be true for 24-hour mode in the hours circle.
	 * @param disappearsOut
	 *            Whether the numbers' animation will have them disappearing out or
	 *            disappearing in.
	 * @param selectionDegrees
	 *            The initial degrees to be selected.
	 * @param isInnerCircle
	 *            Whether the initial selection is in the inner or outer circle. Will be
	...
	public void initialize(Context context, boolean is24HourMode, boolean hasInnerCircle,
		...
		if (mIsInitialized) {
			Log.e(RadialSelectorView.TAG,
					"This RadialSelectorView may only be initialized once.");
			return;
		}
		...
		mSelectionAlpha = RadialSelectorView.SELECTED_ALPHA;
	...
	public void setTheme(Context context, boolean themeDark) {
		...
		if (themeDark) {
			...
			mSelectionAlpha = RadialSelectorView.SELECTED_ALPHA_THEME_DARK;
		} else {
	...
	public int getDegreesFromCoords(float pointX, float pointY, boolean forceLegal,
		...
		if (mHasInnerCircle) {
			if (forceLegal) {
				...
				int distanceToInnerNumber = (int) Math
						.abs(hypotenuse - innerNumberRadius);
				int outerNumberRadius = (int) (mCircleRadius * mOuterNumbersRadiusMultiplier);
				int distanceToOuterNumber = (int) Math
						.abs(hypotenuse - outerNumberRadius);
				isInnerCircle[0] = distanceToInnerNumber <= distanceToOuterNumber;
			} else {
				// Otherwise, if we're close enough to either number (with the space
				// between the
			...
			if (!forceLegal) {
				...
				// The max allowed distance will be defined as the distance from the
				// center of the
	...
	public void onDraw(Canvas canvas) {
		...
		if (!mDrawValuesReady) {
			...
			if (!mIs24HourMode) {
				// We'll need to draw the AM/PM circles, so the main circle will need to
				// have
				// a slightly higher center. To keep the entire view centered vertically,
				// we'll
		...
		if (mForceDrawDot | mSelectionDegrees % 30 != 0) {
			...
			mPaint.setAlpha(RadialSelectorView.FULL_ALPHA);
			canvas.drawCircle(pointX, pointY, mSelectionRadius * 2 / 7, mPaint);
		} else {
			// We're not drawing the dot, so shorten the line to only go as far as the
			// edge of the
			// selection circle.
	...
	public ObjectAnimator getReappearAnimator() {
		...
		float midwayPoint = 0.2f;
		int duration = 500;
		// The time points are half of what they would normally be, because this animation
		// is
		...
		PropertyValuesHolder fadeIn = PropertyValuesHolder.ofKeyframe("alpha", kf0, kf1,
				kf2);
