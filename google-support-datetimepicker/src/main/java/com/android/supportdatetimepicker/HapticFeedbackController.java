...
public class HapticFeedbackController {
    ...
    public HapticFeedbackController(Context context) {
        ...
        mContentObserver = new ContentObserver(null) {
            ...
            public void onChange(boolean selfChange) {
                mIsGloballyEnabled = HapticFeedbackController
                    .checkGlobalSetting(mContext);
            }
        };
    }
    ...
    public void start() {
        ...
        mIsGloballyEnabled = HapticFeedbackController.checkGlobalSetting(mContext);
    ...
    /**
     * Try to vibrate. To prevent this becoming a single continuous vibration, nothing
     * will happen if we have vibrated very recently.
     */
    public void tryVibrate() {
        if (mVibrator != null && mIsGloballyEnabled) {
            long now = SystemClock.uptimeMillis();
            // We want to try to vibrate each individual tick discretely.
            if (now - mLastVibrate >= HapticFeedbackController.VIBRATE_DELAY_MS) {
                mVibrator.vibrate(HapticFeedbackController.VIBRATE_LENGTH_MS);
                mLastVibrate = now;
            }
        }
    }
