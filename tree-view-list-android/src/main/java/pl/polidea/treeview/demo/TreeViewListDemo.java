...
public class TreeViewListDemo extends Activity {
	...
	private static final int[] DEMO_NODES = new int[] {0, 0, 1, 1, 1, 2, 2, 1, 1, 2, 1,
			0, 0, 0, 1, 2, 3, 2, 0, 0, 1, 2, 0, 1, 2, 0, 1};
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			for (int i = 0; i < TreeViewListDemo.DEMO_NODES.length; i++) {
				treeBuilder.sequentiallyAddNextNode((long) i,
						TreeViewListDemo.DEMO_NODES[i]);
			}
			Log.d(TreeViewListDemo.TAG, manager.toString());
			newTreeType = TreeType.SIMPLE;
			newCollapsible = true;
		} else {
			manager = (TreeStateManager) savedInstanceState
		...
		fancyAdapter = new FancyColouredVariousSizesAdapter(this, selected, manager,
				TreeViewListDemo.LEVEL_NUMBER);
	...
	protected void onSaveInstanceState(final Bundle outState) {
		outState.putParcelable("treeManager", manager);
		outState.putSerializable("treeType", treeType);
		outState.putBoolean("collapsible", collapsible);
	...
	public void onCreateContextMenu(final ContextMenu menu, final View v,
		...
		final TreeNodeInfo info = manager.getNodeInfo(id);
