...
public class TreeBuilder {
     ...
     *            child id
     */
    public synchronized void addRelation(final Long parent, final Long child) {
     ...
     *            its level
     */
    public synchronized void sequentiallyAddNextNode(final Long id, final int level) {
        Log.d(TAG, "Adding sequentiall node " + id + " at level " + level);
        if (lastAddedId == null) {
            addNodeToParentOneLevelDown(null, id, level);
        } else {
            if (level <= lastLevel) {
                final Long parent = findParentAtLevel(lastAddedId, level - 1);
     ...
     */
    private Long findParentAtLevel(final Long node, final int levelToFind) {
        Long parent = manager.getParent(node);
        while (parent != null) {
     ...
     */
    private void addNodeToParentOneLevelDown(final Long parent, final Long id,
            final int level) {
    ...
    private void setLastAdded(final Long id, final int level) {
        lastAddedId = id;
        lastLevel = level;
    }
